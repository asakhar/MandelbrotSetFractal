...
void FractalGLWidget::setZChanging(int val) {
  zChanging = val;
}
...
void FractalGLWidget::initializeGL()
{
  ...
  uniformZ = glGetUniformLocation(program, "zStart");
...
void FractalGLWidget::mouseMoveEvent(QMouseEvent *event) {
  if(mouseDragging) {
      if(zChanging) {
          z[0] += ((GLfloat)event->x()/(GLfloat)width() - prevMouse[0])*((GLfloat)width())/100.;
          z[1] += ((GLfloat)event->y()/(GLfloat)height() - prevMouse[1])*((GLfloat)height())/100.;
        }else {
